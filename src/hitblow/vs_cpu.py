...
def setup_vs_cpu(digits: int):
    """CPUと同じ桁数（digits桁）でユーザーの数字を設定する"""
...
def play_cpu_turn(digits: int) -> bool:
    """CPUがプレイヤーの数字（digits桁）を予想する。CPUが正解したら True を返す"""
    global _candidates
